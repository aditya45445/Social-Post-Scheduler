    ...
    if (accounts.length === 0) {
        return (
            <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center py-20 px-6">
                ...
                <p className="text-sm text-slate-500 mt-1 max-w-xs text-center">Connect your first social platform to start scheduling and automating your content.</p>
            </div>
        )
    }
    return (
            ...
            {accounts.map((account, index) => {
                ...
                return (
                    <div key={index} className="group bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4 hover:border-slate-300 transition-all">
                        ...
                        <button onClick={() => handleDisconnect(account._id)} title='Disconnected Account' className="ml-2 p-1.5 rounded-lg text-slate-300 group-hover:text-red-500 transition-all">
